...
2. **Spec generation** (`stepman.WriteStepSpecToFile` via `parseStepCollection` in `stepman/util.go`): walks `collection/steps/**/step.yml`, parses + audits each one, joins per-step `assets/*` URLs against `assets_download_base_uri`, emits `index/spec.json` (~24 MB) and `index/slim-spec.json` (latest-version-only, ~2 MB).
...
- **`steps/` is the source of truth.** Every file under `index/` can be deterministically regenerated by walking `steps/`. If `index/` is corrupt, regenerate; no data is lost.
- **`index/` is a query-optimized projection.** It exists to spare clients the cost of walking the tree.
...
|---|---|---|
| `/index/*` | `Cache-Control: public, max-age=60, must-revalidate` + `ETag` | Index files change on every release; short revalidation keeps things fresh, ETag avoids unnecessary transfer when unchanged. |
...
|---|---|---|---|
| **Fixed** | `1.2.3` | `steps/<id>/1.2.3/step.json` (1 fetch) | Never touches `index/`. A 404 is the canonical "no such version" signal. Once fetched, the file is immutable for a year — repeat builds re-validate nothing. |
| **Latest** | `""` / `latest` | `index/steps/<id>/latest.json` → `steps/<id>/<resolved>/step.json` (2 fetches) | Read `latest` field, then the resolved `step.json`. |
| **MajorLocked** | `1.x.x` or `1` | `index/steps/<id>/latest.json` → `steps/<id>/<resolved>/step.json` (2 fetches) | **Same file as Latest** — read `latest_by_major["1"]` instead of `latest`. Shared cache key with the Latest route is a real win. |
| **MinorLocked** | `1.2.x` | `index/steps/<id>/versions.json` → `steps/<id>/<resolved>/step.json` (2 fetches) | Client filters the version list for matching `major.minor`, picks highest patch. `versions.json` is larger (~300 B gz median), so we keep MinorLocked off the small `latest.json`. |
...
proactive validation list can fetch `index/step_ids.json` once per
session.)
