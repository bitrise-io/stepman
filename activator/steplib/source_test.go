...
func TestActivateStepSourceWithAPI_MissingSourceGitFails(t *testing.T) {
	...
	t.Run("nil source", func(t *testing.T) {
		err := activateStepSourceWithAPI(client, "hello-step", "2.0.0", nil, t.TempDir(), log, httpfetch.NewClient(log))
		require.Error(t, err)
	...
	t.Run("empty git URL", func(t *testing.T) {
		err := activateStepSourceWithAPI(client, "hello-step", "2.0.0",
			&models.StepSourceModel{Git: ""}, t.TempDir(), log, httpfetch.NewClient(log))
		require.Error(t, err)
		require.Contains(t, err.Error(), "source git")
	})
}
...
func TestActivateStepSourceWithAPI_NoDownloadLocationFails(t *testing.T) {
	...
	err := activateStepSourceWithAPI(client, "hello-step", "2.0.0",
		&models.StepSourceModel{Git: "https://github.com/example/hello-step.git"}, t.TempDir(), log, httpfetch.NewClient(log))
