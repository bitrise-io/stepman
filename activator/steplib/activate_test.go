...
func TestActivateStep_ResolvesExactVersion(t *testing.T) {
	...
	resolved, err := ActivateStep(id, destination, stepYML, log, false, steplibrary.New(log, srv.URL), newFakeExecutableFetcher(t, nil))
...
func TestActivateStep_ResolvesMajorLock(t *testing.T) {
	...
	resolved, err := ActivateStep(id, t.TempDir(), filepath.Join(t.TempDir(), "current_step.yml"), log, false, steplibrary.New(log, srv.URL), newFakeExecutableFetcher(t, nil))
	require.NoError(t, err)
...
func TestActivateStep_NonexistentVersionFails(t *testing.T) {
	...
	_, err := ActivateStep(id, t.TempDir(), filepath.Join(t.TempDir(), "current_step.yml"), log, false, steplibrary.New(log, srv.URL), newFakeExecutableFetcher(t, nil))
...
func TestActivateStep_NoExecutable_ActivatesSource(t *testing.T) {
	...
	resolved, err := ActivateStep(id, destination, filepath.Join(t.TempDir(), "current_step.yml"), log, false, steplibrary.New(log, srv.URL), newFakeExecutableFetcher(t, nil))
	require.NoError(t, err)
...
func TestActivateStep_ExecutableDownloadFails_FallsBackToSource(t *testing.T) {
	...
	fetcher := newFakeExecutableFetcher(t, errFakeDownload)
